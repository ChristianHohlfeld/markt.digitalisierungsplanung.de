...
const SCHEMA_URL = process.env.CANONICAL_SCHEMA_URL || "https://raw.githubusercontent.com/ChristianHohlfeld/digitalisierungsplanung.de/main/contracts/preset-package.schema.json";
const REGISTRY_PATH = resolve(root, process.env.REGISTRY_PATH || "data/registry.json");
...
const manifestMatch=path.match(/^\/api\/packages\/([^/]+)\/manifest$/);if(req.method==="GET"&&manifestMatch){if(!contractReady(res))return;const r=registry.get(decodeURIComponent(manifestMatch[1]));if(!r||r.status!=="published")return send(res,404,{error:"not_found"});if(!contract.validate(r.manifest).ok)return send(res,503,{error:"registry_contract_violation"});return send(res,200,r.manifest);}
const downloadMatch=path.match(/^\/api\/packages\/([^/]+)\/download$/);if(req.method==="POST"&&downloadMatch){if(!contractReady(res))return;const r=registry.get(decodeURIComponent(downloadMatch[1]));if(!r||r.status!=="published")return send(res,404,{error:"not_found"});if(!contract.validate(r.manifest).ok)return send(res,503,{error:"registry_contract_violation"});await registry.countDownload(r.manifest.id);return send(res,200,{manifest:r.manifest});}
